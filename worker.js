...
var ALLOWED_ORIGIN = "https://makes971-hash.github.io";
var CACHE_TTL = 60;
var REGIONS = [
...
function getCached(key) {
  ...
  if (Date.now() - entry.time > CACHE_TTL * 1e3) {
    cache.delete(key);
    return null;
  }
  return entry.data;
    ...
    try {
      ...
      if (path === "/health") {
        ...
        return new Response(JSON.stringify({
          ...
          anthropic_key_loaded: !!env.ANTHROPIC_API_KEY,
      ...
      if (path === "/powerbi" || path.startsWith("/powerbi/")) {
        ...
        __name(teslaPbiGet, "teslaPbiGet");
          ...
          for (const site of energySites) {
            ...
            try {
              ...
              if (liveResp.ok) {
                ...
                if (live) {
                  row.solar_power_kw = +((live.solar_power || 0) / 1e3).toFixed(3);
                  row.load_power_kw = +((live.load_power || 0) / 1e3).toFixed(3);
                  row.grid_power_kw = +((live.grid_power || 0) / 1e3).toFixed(3);
                  row.battery_power_kw = +((live.battery_power || 0) / 1e3).toFixed(3);
                  row.battery_pct = live.percentage_charged != null ? +Number(live.percentage_charged).toFixed(1) : 0;
        ...
        if (subPath === "/history") {
          ...
          const rows = series.map((d) => ({
            ...
            solar_power_kw: +((d.solar_power || 0) / 1e3).toFixed(3),
            load_power_kw: +((d.load_power || 0) / 1e3).toFixed(3),
            grid_power_kw: +((d.grid_power || 0) / 1e3).toFixed(3),
            battery_power_kw: +((d.battery_power || 0) / 1e3).toFixed(3)
          }));
          const result = {
            site_id: siteId,
            period,
            ...
            try {
              ...
              if (lastResp.ok || lastResp.status !== 429 && lastResp.status !== 500 && lastResp.status !== 502 && lastResp.status !== 503) {
                if (request.method === "GET" && lastResp.ok) {
                  const data2 = await lastResp.json();
                  const cacheKey = teslaPath + url.search;
